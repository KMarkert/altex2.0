...
def etl(sensor, workingDir, outDir, startTime=None, endTime=None, overwrite=False, maxWorkers=5,
        spatialFilter=None,cleanup=False):

    workingDir = Path(workingDir) 
    if not os.path.exists(str(workingDir.resolve())):
    ...
    # save the transformed dataset to a compressed parquet file
    outDir = Path(outDir) 
    if not os.path.exists(str(outDir.resolve())):
        outDir.mkdir(parents=True)

    outFile = str(outDir / f'{sensor}_{startTime.replace("-","")}.parq.gz')
    df.to_parquet(outFile,engine='fastparquet',compression='gzip',index=None)

    if cleanup:
        trash = workingDir.glob('*.*')
        for file in trash:
            os.remove(file)
    return
